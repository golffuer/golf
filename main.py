...
bot = telebot.TeleBot('5228642369:AAGQJwrL9paeUtQ1U4T-CqZOlJ3Bb0haZsw')  # Создаем экземпляр бота
            ...
            for _ in range(5):
                ...
                for user in Users.activeUsers.values():
                    ...
                    if user.id != cur_user.id and user.get_cur_menu() in botGames.GameRPS.values:
                        ...
                        bot.send_message(chat_id, text="ЗА АЛЬЯНС!!!!!!! - @{enemy}".format(enemy=user.userName))
                ...
                sleep(1)
            if not text_game:
                bot.send_message(chat_id, text="ЭЛЬФ НЕ НАЙДЕН!!!!!!")
                img = open('котик.jpg', 'rb')
...
def goto_menu(chat_id, name_menu):
    ...
    if target_menu != None:
        ...
        if target_menu.name == "Игра в 21":
            ...
            text_game = "<b>ТЫ ПОБЕДИШБ ЕСЛИ ТЫ:</b>\n" \
                        "1. ОРК\n" \
                        "2. ОРК\n" \
                        "3. ОРК"
...
def send_help(chat_id):
    ...
    btn1 = types.InlineKeyboardButton(text="Напишите автору", url="https://t.me/gggaaavv")
    markup.add(btn1)
    img = open('дд.jpg', 'rb')
    bot.send_photo(chat_id, img, reply_markup=markup)

    bot.send_message(chat_id, "АРМИЯ:")
